            ...
            <div class="picker-wheel" v-for="(wheel, index) in pickerData" :key="index">
              <ul class="wheel-scroll">
                <li class="wheel-item" v-for="(item, index) in wheel" :key="index">
                  <span v-if="typeof item === 'object'">{{ item.label }}</span>
  ...
  methods: {
      ...
      anchor = anchor.map((item, i) => {
        ...
        if (item.index) {
          ...
          index =
            this.pickerData &&
            this.pickerData[i] &&
            this.pickerData[i].indexOf(item.value) > -1
              ? this.pickerData[i].indexOf(item.value)
              : 0;
        } else {
          index = item;
        }
    ...
    _getCurrentValue() {
      ...
      this.wheels.forEach((wheel, i) => {
        ...
        if (typeof this.pickerData[i][j] === "object") {
          value.push({
            index: j,
            value: this.pickerData[i][j].value
          });
        } else {
          value.push({
            index: j,
    ...
    _updatePickerData(wheelIndex = 0) {
      ...
      while (data) {
        if (i >= wheelIndex) {
          ...
          this.pickerAnchor[i] =
            wheelIndex === 0
              ? this.pickerAnchor[i] < data.length
                ? this.pickerAnchor[i] || 0
                : 0
